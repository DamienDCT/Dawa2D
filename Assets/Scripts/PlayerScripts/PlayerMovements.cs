...
public class PlayerMovements : MonoBehaviour
{
 ...
 //   private float _fallSpeedYDampingChangeThreshold;
    ...
    private void Start()
    {
        ...
        IsFacingRight = true;
        /*        InputController.Instance.OnJumpPressed += OnJumpInput;
                InputController.Instance.OnJumpReleased += OnJumpUpInput;*/
        amountJumps = 0;
    }
...
/*        if (RB.linearVelocityY < _fallSpeedYDampingChangeThreshold && !CameraManager.Instance.IsLerpingYDamping && !CameraManager.Instance.LerpedFromPlayerFalling)
        ...
        _moveInput = InputController.Instance.MovementVector;

        if (_moveInput.x != 0)
            CheckDirectionToFace(_moveInput.x > 0);

/*        if (InputController.Instance.Ju)
        {
            OnJumpInput();
        }
        ...
        {
            // Reset la direction du dernier walljump
            _lastWallJumpDir = 0;
            IsWallJumping = false; // sécurité au cas où
        }
        #endregion
    ...
    #region JUMP METHODS
    public void Jump(float force = -1f, bool isDouble = false)
    {
    ...
    {
        return LastOnGroundTime < 0 && amountJumps < Data.maxJumps;
    }

    private bool CanWallJump()
    {
        // Doit appuyer saut, être collé à un mur, pas au sol
        if (LastPressedJumpTime <= 0 || LastOnWallTime <= 0 || LastOnGroundTime > 0)
            return false;

        // Mur de droite → on interdit si le dernier saut venait déjà de ce mur
        if (LastOnWallRightTime > 0 && _lastWallJumpDir != -1)
            return true;

        // Mur de gauche → on interdit si le dernier saut venait déjà de ce mur
        if (LastOnWallLeftTime > 0 && _lastWallJumpDir != 1)
            return true;
